let guests=[], scanner=null, busy=false, protocolToken=null, protocolName='', protocolRole='protocol', chiefTeam=[], chiefTasks=[], myTasks=[];
...
async function enter(){A('#login').classList.add('hidden');A('#reception').classList.remove('hidden');A('#chiefTools')?.classList.toggle('hidden',protocolRole!=='chief');A('#myTasks')?.classList.remove('hidden');await loadGuests();await loadMyTasks();if(protocolRole==='chief')await loadChiefTools();A('#code').focus()}
...
async function loadChiefTools(){
 ...
 chiefTeam=team.data||[];chiefTasks=tasks.data||[];renderChiefTools();
}
function renderChiefTools(){
 ...
 if(list)list.innerHTML=workers.map(p=>`<div class="chief-task"><div class="protocol-avatar">${esc((p.full_name||'?').slice(0,1).toUpperCase())}</div><div class="chief-task-main"><b>${esc(p.full_name)}</b><small>${esc(p.access_code)} · ${p.active?'Activo':'Inactivo'}</small></div></div>`).join('')||'<div class="empty-state">Ainda não existem protocolos.</div>';
 if(sel)sel.innerHTML=workers.map(p=>`<option value="${p.id}">${esc(p.full_name)}</option>`).join('')||'<option value="">Sem protocolos</option>';
 if(tasks)tasks.innerHTML=chiefTasks.map(t=>`<div class="chief-task"><div class="chief-task-main"><b>${esc(t.protocol_name)} — ${esc(t.title)}</b><small>${esc(t.description||'Sem descrição')}</small></div><div class="chief-task-actions"><button class="icon-btn" onclick="chiefToggleTask(${t.id},${!t.completed})">${t.completed?'Reabrir':'Concluir'}</button><button class="icon-btn danger" onclick="chiefDeleteTask(${t.id})">Remover</button></div></div>`).join('')||'<div class="empty-state">Ainda não existem tarefas atribuídas.</div>';
}
