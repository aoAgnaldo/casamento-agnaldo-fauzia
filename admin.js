...
function openProtocolModal(p=null){A('#protocolModalTitle').textContent=p?'Editar protocolo':'Novo protocolo';A('#protocolModalDescription').textContent=p?'Actualize os dados de acesso deste protocolo.':'Crie um acesso individual para quem vai trabalhar na recepção.';A('#protocolSubmitBtn').textContent=p?'Guardar alterações':'Criar acesso';A('#protocolPin').required=!p;A('#protocolPin').value='';A('#protocolPinHint').textContent=p?'Deixe o PIN vazio para manter o actual. Se quiser corrigir ou trocar o PIN, introduza um novo PIN.':'Mínimo de 4 dígitos.';A('#protocolName').value=p?.full_name||'';A('#protocolPhone').value=p?.whatsapp||'';A('#protocolCode').value=p?.access_code||'';A('#protocolForm').dataset.id=p?.id||'';A('#protocolFormMsg').classList.add('hidden');A('#protocolModal').classList.remove('hidden');document.body.classList.add('modal-scroll-lock');setTimeout(()=>A('#protocolName').focus(),80)}
...
A('#protocolForm').onsubmit=async e=>{e.preventDefault();const id=A('#protocolForm').dataset.id||'',name=A('#protocolName').value.trim(),phone=A('#protocolPhone').value.trim(),code=A('#protocolCode').value.trim(),pin=A('#protocolPin').value.trim();if(!name||!code||(id===''&&!pin)){showMsg(A('#protocolFormMsg'),'Preencha o nome, código e PIN.');return}const fn=id?'admin_update_protocol':'admin_create_protocol';const args=id?{p_id:id,p_full_name:name,p_whatsapp:phone,p_access_code:code,p_pin:pin||null}:{p_full_name:name,p_whatsapp:phone,p_access_code:code,p_pin:pin};const r=await supabaseClient.rpc(fn,args);if(r.error){showMsg(A('#protocolFormMsg'),r.error.message);return}A('#protocolModal').classList.add('hidden');document.body.classList.remove('modal-scroll-lock');await refresh();toast(id?'Protocolo actualizado.':'Acesso de protocolo criado.')};
